package com.fds.flex.core.inspectionmgt.entity.C_Model;

import com.fasterxml.jackson.annotation.JsonIgnoreProperties;
import com.fds.flex.common.utility.string.StringPool;
import com.fds.flex.modelbuilder.constant.DBConstant;
import com.fds.flex.modelbuilder.entity.BaseCategory;
import lombok.Getter;
import lombok.Setter;
import org.springframework.data.mongodb.core.mapping.Document;

@Setter
@Getter
@Document(collection = DBConstant.C_CHUC_DANH_DOAN)
@JsonIgnoreProperties(ignoreUnknown = true)
public class ChucDanhDoan extends BaseCategory<ChucDanhDoan> {

    private static final long serialVersionUID = 1L;

    public ChucDanhDoan() {
        super(false);
        super.setType(getType());
    }

    public ChucDanhDoan(boolean isUpdate) {
        super(isUpdate);
        super.setType(getType());

    }

    @Override
    public String getType() {
        return DBConstant.C_CHUC_DANH_DOAN;
    }

    @Override
    public void setType(String type) {
        super.setType(DBConstant.C_CHUC_DANH_DOAN);
    }

    @Override
    public String getPrimKey() {

        String primKey = this.get_id() != null ? this.get_id().toHexString() : StringPool.BLANK;

        super.setPrimKey(primKey);

        return primKey;
    }

    @Getter
    public enum ChucDanh {
        TruongDoan("01", "Trưởng đoàn"),
        ToTruong("02", "Tổ trưởng"),
        PhoTruongDoan("03", "Phó trưởng đoàn"),
        ThuKyDoan("04", "Thư ký đoàn"),
        ThanhVien("05", "Thành viên");

        public String maMuc = StringPool.BLANK;
        public String tenMuc = StringPool.BLANK;

        ChucDanh(String maMuc, String tenMuc) {
            setMaMuc(maMuc);
            setTenMuc(tenMuc);
        }

        public void setMaMuc(String maMuc) {
            this.maMuc = maMuc;
        }

        public void setTenMuc(String tenMuc) {
            this.tenMuc = tenMuc;
        }

    }

}
