...
import com.fds.flex.core.cadmgt.service.statistic.TongHopKetQuaXuLyDonToCaoService;
...
@Service
@Slf4j
public class TongHopKetQuaXuLyDonToCaoServiceImpl implements TongHopKetQuaXuLyDonToCaoService {
    @Autowired
    ...
    public Map<String, Long> queryValue12(List<String> coQuanQuanLy, long ca_from, long ca_to) {
        ...
        criteria.add(new Criteria().orOperator(criteriaLv1));
        criteria.add(Criteria.where("LinhVucChuyenNganh.MaMuc").is(LinhVucChuyenNganh.LinhVuc.CongChucCongVu.getMaMuc()));
        searchOperationList.add(Aggregation.match(new Criteria().andOperator(criteria)));
        searchOperationList.add(Aggregation.group("CoQuanQuanLy.MaDinhDanh").count().as(Constant._SO_LUONG));
        Aggregation agg = newAggregation(searchOperationList);
        AggregationResults<Document> resultCountDB = mongoTemplate.aggregate(agg, VuViecDonThu.class,
    ...
    public Map<String, Long> queryValue14(List<String> coQuanQuanLy, long ca_from, long ca_to) {
        ...
        criteria.add(new Criteria().orOperator(criteriaLv1));
        criteria.add(Criteria.where("LinhVucChuyenNganh.MaMuc").is(LinhVucChuyenNganh.LinhVuc.ThamNhung.getMaMuc()));
        searchOperationList.add(Aggregation.match(new Criteria().andOperator(criteria)));
        searchOperationList.add(Aggregation.group("CoQuanQuanLy.MaDinhDanh").count().as(Constant._SO_LUONG));
        Aggregation agg = newAggregation(searchOperationList);
        AggregationResults<Document> resultCountDB = mongoTemplate.aggregate(agg, VuViecDonThu.class,
    ...
    public Map<String, Long> queryValue20(List<String> coQuanQuanLy, long ca_from, long ca_to) {
        ...
        criteria.add(Criteria.where("TrangThaiDuLieu.MaMuc").is(TrangThaiDuLieu.TrangThai.ChinhThuc.getMaMuc()));
        criteria.add(Criteria.where("LoaiVuViecDonThu.MaMuc").is(LoaiVuViecDonThu.Loai.ToCao.getMaMuc()));
        criteria.add(new Criteria().orOperator(Criteria.where("NgayKetThuc").exists(false), Criteria.where("NgayKetThuc").gt(ca_to)));
        criteria.add(new Criteria().orOperator(
                new Criteria().andOperator(Criteria.where("ThoiHanThucHien").gt(ca_to), Criteria.where("GiaHanThucHien").exists(false)),
                Criteria.where("GiaHanThucHien").gt(ca_to)));
        criteria.add(new Criteria().andOperator(Criteria.where("ThongBaoKetLuan").exists(true), Criteria.where("ThongBaoKetLuan.MaDinhDanh").ne(StringPool.BLANK)));
